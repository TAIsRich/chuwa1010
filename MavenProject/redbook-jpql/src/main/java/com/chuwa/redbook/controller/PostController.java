...
public class PostController {
    ...
    @GetMapping("jpql-index/{id}")
    public ResponseEntity<PostDto> ggetPostByIdJPQLNamedParameter(@PathVariable(name = "id") long id,
                                                               @RequestParam(value = "title", required = false) String title){
        return ResponseEntity.ok(postService.getPostByIdJPQLNamedParameter(id, title));
    }

    @GetMapping("jpql-index/{id}")
    public ResponseEntity<PostDto> getPostByIdSQLIndexParameter(@PathVariable(name = "id") long id,
                                                               @RequestParam(value = "title", required = false) String title){
        return ResponseEntity.ok(postService.getPostByIdSQLIndexParameter(id, title));
    }

    @GetMapping("jpql-index/{id}")
    public ResponseEntity<PostDto> getPostByIdSQLNamedParameter(@PathVariable(name = "id") long id,
                                                               @RequestParam(value = "title", required = false) String title){
