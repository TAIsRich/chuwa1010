...
//				.collect(Collectors.toList());
		...
		List<Product >result = orderRepo.findAll()
				...
				.filter(o -> o.getOrderDate().isEqual(LocalDate.of(2021, 3, 15)))
				.peek(o -> System.out.println(o.toString()))
				.flatMap(o ->o.getProducts().stream())// Note: using flatmap, we need to specify the result as .stream()
				.distinct()
				.collect(Collectors.toList());
...
//				.sum();
		double result = orderRepo.findAll()
				...
				.filter(o -> o.getOrderDate().compareTo(LocalDate.of(2021, 3, 1)) < 0)
				.flatMap(o -> o.getProducts().stream())
				.mapToDouble(Product::getPrice)// same as .mapToDouble(p -> p.getPrice())
				.sum();
...
//						);
		Map<Long, Integer> result = orderRepo.findAll()
				...
				.collect(
						Collectors.toMap(
								Order::getId, // same as order -> order.getId()
								order -> order.getProducts().size())
				);
...
//				.collect(Collectors.groupingBy(Order::getCustomer));
		...
		// a customer has a list of orders, and now we have many customers, so create a map as customer->orders
		Map<Customer, List<Order>> result = orderRepo.findAll()
				.stream()
				.collect( // groupingBy() takes the key element as input, here our key is customer
						Collectors.groupingBy(Order::getCustomer) // (o -> o.getCustomer)
				);
...
//						);
		Map<String, List<String>> result = productRepo.findAll()
				.stream()
				.collect(
						// another input form of groupingBy is just (key, value)
						// now we need products'name as value rather than Product Object,
						// that's why we use Collectors.mapping() to convert the product object to product name
						Collectors.groupingBy(
								Product::getCategory,
								Collectors.mapping(p -> p.getName(), Collectors.toList())
								// note that we declare this map as <String, List<String>>, that's why we use toList()
						)
				);
		...
		Map<String, Optional<Product>> result = productRepo.findAll()
				...
				.collect(
						Collectors.groupingBy(
								Product::getCategory,
								Collectors.maxBy(Comparator.comparing(Product::getPrice))
								// By Providing a comparator for product price, maxBy() can get the product Object with largest value
						)
				);
		long endTime = System.currentTimeMillis();
