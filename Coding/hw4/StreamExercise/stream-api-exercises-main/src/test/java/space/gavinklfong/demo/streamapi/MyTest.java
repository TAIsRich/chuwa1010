...
public class MyTest {
    ...
    public void exercise1() {
        ...
        books.forEach(p -> log.info(p.toString()));
    }

    @Test
    @DisplayName("Obtain a list of product with category = \"Books\" and price > 100 (using Predicate chaining for filter)")
    public void exercise1a() {
        Predicate<Product> booksProductFilter = e -> e.getCategory().equalsIgnoreCase("Books");
        Predicate<Product> priceFilter = e -> e.getPrice() > 100;

        long startTime = System.currentTimeMillis();

        List<Product> books = productRepo.findAll().stream()
                .filter(e -> booksProductFilter .and (priceFilter).test(e) )
                .collect(Collectors.toList());
        long endTime = System.currentTimeMillis();
    ...
    public void exercise1b() {
        BiPredicate<Product, String> booksFilter = (product, category) -> product.getCategory().equalsIgnoreCase(category);
        long startTime = System.currentTimeMillis();
        List<Product> books = productRepo.findAll().stream()
                .filter(e -> booksFilter.test(e, "books") && e.getPrice() > 100 )
                .collect(Collectors.toList());
        long endTime = System.currentTimeMillis();
    ...
    @Test
    @DisplayName("Get 3 cheapest products of \"Books\" category")
...
//                .min(Comparator.comparing(Product::getPrice));
    ...
    public void exercise7() {
        ...
        List<Product> result = orderRepo.findAll().stream()
                ...
                //.peek(o -> System.out.println(o.toString()))//list the orders
    ...
    public void exercise8a() {
        BiFunction<Double, Product, Double> accumlator = (acc, product) -> acc + product.getPrice();
        long startTime = System.currentTimeMillis();

        double result = orderRepo.findAll().stream()
                .filter(e -> e.getOrderDate().compareTo(LocalDate.of(2021,2,1))>= 0)
                .filter(e -> e.getOrderDate().compareTo(LocalDate.of(2021,2,28))<= 0)
                .flatMap(e -> e.getProducts().stream())
                .reduce(0D, accumlator, Double::sum);
        long endTime = System.currentTimeMillis();
