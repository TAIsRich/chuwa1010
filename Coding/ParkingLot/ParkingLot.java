...
//create a Parking Lot with the Number of levels and the slots in each of the given levels
public class ParkingLot {
    private Lvl[] lvls;
    private final int NUM_LVLS=2;
    private int numberSlots;

    //key contains the company name and the value is the vehicles present in the list
    ...
    // a method for handing the scenario when a vehicle is parked
    ...
    //method for handing the scenario when a vehicle leaves
    public boolean leave(Vehicle vh, int lvl){
        System.out.println("-----------------------");
        lvls[lvl].slotFreed();
        System.out.println("Slot freed from " +lvl+ " and exited " +vh.licPlate + " of "+ vh.companyName);
        ArrayList<String> vhList= this.CompanytoVeh.get(vh.companyName);

        //check if the vehicle is present in the list or not, if present, remove the vehicle from the list
        if(vhList.contains(vh.licPlate)){
            vhList.remove(vh.licPlate);
        }
        return true;
    }
    // method for displaying the list of vehicle of the give company
    public void CompanyParked(String companyName){
        System.out.println("-----------------------");
        ArrayList<String> vhList= this.CompanytoVeh.get(companyName);
        System.out.println("The vehicles of "+ companyName + " : ");
