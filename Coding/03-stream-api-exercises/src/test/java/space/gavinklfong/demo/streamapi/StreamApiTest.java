...
public class StreamApiTest {
	...
	public void exercise1() {
		...
		List<Product> result = productRepo.findAll().stream().filter(p -> p.getCategory().
				equalsIgnoreCase("Book")).filter(p -> p.getPrice() > 100).collect(Collectors.toList());
	...
	public void exercise3() {
		...
		List<Product> result = productRepo.findAll()
				.stream()
				.filter(p -> p.getCategory().equalsIgnoreCase("Toys")).map(p->p.withPrice(p.getPrice() * 0.9)).collect(Collectors.toList());
